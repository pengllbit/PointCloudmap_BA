...
public:
  ...
  void init_storage(int total_layer_num_)
  {
    ...
    //#ifdef FULL_HESS 的含义是：如果在之前的代码中通过 #define FULL_HESS 定义了宏 FULL_HESS，
    //则编译预处理器将处理 #ifdef 和 #endif 之间的代码块。
    //如果未定义 FULL_HESS，则这部分代码会被编译预处理器忽略，不参与实际编译。
    #ifdef FULL_HESS
  ...
  void init_parameter(int pose_size_ = 0)
  {
    ...
    tail = (pose_size - WIN_SIZE) % GAP;   // GAP 步长， 
    gap_num = (pose_size - WIN_SIZE) / GAP;    //gap_num 每层滑窗的数量
    last_win_size = pose_size - GAP * (gap_num+1);   //最后一个滑窗大小
    part_length = ceil((gap_num+1)/double(thread_num));   //ceil向上取整
    ...
    // 初始化了这些参数
    // layer_num-第几层 ，线程数，层的帧数，最大迭代次数==10， part_length
    // gap_num 滑窗数量， voxel_size==4  eigen_ratio--平面特征阈值
...
public:
  ...
  {
    total_layer_num = total_layer_num_;   //total_layer_num_==3 层
    ...
    // 这里只将pose赋给了layer[0].pose_vec
    // 后面 update_next_layer_state 时会更将pose赋给下一个layer[i]
    layers[0].pose_vec = mypcl::read_pose(data_path);
