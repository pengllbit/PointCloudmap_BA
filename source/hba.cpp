...
// 将位置相近的点合并到一个格子 OCTO_TREE_NODE 里
                ...
                double voxel_size, int window_size, float eigen_ratio)
{
  ...
  for(PointType& p_c: feat_pt.points)
  {
    ...
    Eigen::Vector3d pvec_tran = q * pvec_orig + t;   // 世界坐标系

    for(int j = 0; j < 3; j++)
    {
      loc_xyz[j] = pvec_tran[j] / voxel_size;       // 体素位置
      if(loc_xyz[j] < 0) loc_xyz[j] -= 1.0;
    }

    VOXEL_LOC position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);
    auto iter = feat_map.find(position);    // 用哈希查找与体素位置匹配的键
    //如果找到了匹配的键，则将原始点云坐标和新的点云坐标分别添加到相应的向量中。
    ...
    //如果没有找到匹配的键，则创建一个新的OCTO_TREE_ROOT对象，
    //并将原始点云坐标和新的点云坐标分别添加到其vec_orig和vec_tran向量中。
    //然后，将新的 OCTO_TREE_ROOT 对象添加到feat_map中
    else
    {
...
void parallel_comp(LAYER& layer, int thread_id, LAYER& next_layer)
{
  ...
  {
    vector<pcl::PointCloud<PointType>::Ptr> src_pc, raw_pc;  // 点云
    ...
    if(layer_num != 1)
      for(int j = i*GAP; j < i*GAP+WIN_SIZE; j++)
        // 将 layer.pcds[j] 的共享指针赋值给src_pc[j-i*GAP]
        src_pc[j-i*GAP] = (*layer.pcds[j]).makeShared();

    // 是一种与平台相关的无符号整数类型，通常被用于表示内存大小、数组索引等非负整数值。
    size_t mem_cost = 0;
    for(int loop = 0; loop < layer.max_iter; loop++)
    {
      if(layer_num == 1)
        for(int j = i*GAP; j < i*GAP+WIN_SIZE; j++)
        {
          if(loop == 0)
          {
            ...
            // pll 适配
            mypcl::loadPCD(layer.data_path, pc, layer.pose_vec[j].timestamp);
            //mypcl::loadPCD(layer.data_path, pcd_name_fill_num, pc, j, "pcd/");
            raw_pc[j-i*GAP] = pc;
          }
          src_pc[j-i*GAP] = (*raw_pc[j-i*GAP]).makeShared();  // 用智能指针赋值
        }
      ...
      {
        // downsample_size == 0.1  downsample_voxel---降采样
...
int main(int argc, char** argv)
{
  ...
  for(int i = 0; i < total_layer_num-1; i++)   //total_layer_num == 3 层
  {
    std::cout<<"---------------------"<<std::endl;
    distribute_thread(hba.layers[i], hba.layers[i+1]);
    hba.update_next_layer_state(i);   //更新下一层的姿态向量
  }
